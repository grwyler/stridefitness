...
export const strideUsers=sqliteTable('stride_users',{id:text('id').primaryKey(),accountType:text('account_type').notNull(),createdAt:text('created_at').notNull(),lastActiveAt:text('last_active_at').notNull(),convertedAt:text('converted_at')});
export const authIdentities=sqliteTable('auth_identities',{provider:text('provider').notNull(),providerSubject:text('provider_subject').notNull(),userId:text('user_id').notNull(),email:text('email'),displayName:text('display_name'),createdAt:text('created_at').notNull(),lastUsedAt:text('last_used_at').notNull()},table=>[index('idx_auth_identities_user').on(table.userId)]);
export const emailLoginChallenges=sqliteTable('email_login_challenges',{tokenHash:text('token_hash').primaryKey(),email:text('email').notNull(),expiresAt:text('expires_at').notNull(),consumedAt:text('consumed_at'),requestedAt:text('requested_at').notNull()});
