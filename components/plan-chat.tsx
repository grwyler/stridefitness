...
export function PlanChat({data,state,onApply,onReset,onView}:{data:Data;state:PlannerState;onApply:(plan:GeneratedPlan,messages:PlanMessage[])=>void;onReset:()=>void;onView:()=>void}){
 const [input,setInput]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState(''),[signInUrl,setSignInUrl]=useState('');const pending=useRef(false);const controller=useRef<AbortController|null>(null);const log=useRef<HTMLDivElement>(null);
 ...
 async function send(){
  ...
  pending.current=true;setBusy(true);setError('');setSignInUrl('');
  ...
  try{
   ...
   const body=await response.json();if(response.status===401&&body.signInUrl?.startsWith('/signin-with-chatgpt?'))setSignInUrl(body.signInUrl);if(!response.ok)throw new Error(body.error||'Unable to create your plan. Try again.');
