...
export function SetCoach({data,workout,exercise}:{data:Data;workout:Workout;exercise:Exercise}){
 ...
 const next={...data,workouts:data.workouts.map(w=>w.id===workout.id?{...w,entries:w.entries.map(e=>e.exerciseId===exercise.id?{...e,sets:e.sets.map(s=>s.id===advice.setId?{...s,weight:advice.weight,reps:advice.reps,targetWeight:advice.weight,targetReps:advice.reps}:s)}:e)}:w)};
 ...
 return <div className="set-coach" aria-live="polite"><div><Sparkles size={17}/><strong>Coach · Next set</strong></div><p>{advice.reason}</p><strong>{advice.weight} lb × {advice.reps} reps</strong><div className="button-group">{advice.changed?<><button className="primary" disabled={saving} onClick={()=>void apply()}>{saving?'Saving…':'Apply to next set'}</button><button className="text-button" disabled={saving} onClick={()=>setDismissed(key)}>Keep my target</button></>:<span className="muted">Your next target is on track.</span>}</div>{error&&<p role="alert">{error}</p>}</div>
}
