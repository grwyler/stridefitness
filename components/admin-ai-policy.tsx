...
export function AdminAiPolicy({initialIncluded}:{initialIncluded:boolean}){
 ...
 return <section className="panel" style={{marginBottom:24}}><h2>AI access for new accounts</h2><p>{included?'New users receive complimentary AI, funded by your connection.':'New users need their own API key. You can grant complimentary AI individually below.'}</p><button className="secondary" disabled={busy} onClick={()=>void update()}>{busy?'Saving…':included?'Stop including AI for new accounts':'Include AI for new accounts'}</button><p className="form-help">Existing accounts keep their access. Revoking complimentary AI never blocks a user’s personal key.</p><p className="form-help">Subscriptions are not enabled yet. Payment setup and pricing are still required.</p>{message&&<p role="status">{message}</p>}{error&&<p className="plan-error" role="alert">{error}</p>}</section>
}
