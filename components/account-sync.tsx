...
export type AccountSyncHandle={stage:(action:Operation['action'],before:Data,next:Data,label:string)=>string|null;apply:(id:string)=>Promise<boolean>;saveNow:(next:Data)=>Promise<boolean>};
export const AccountSync=forwardRef<AccountSyncHandle,{data:Data;onLoad:(data:Data)=>void;onView?:(target:string)=>void}>(function AccountSync({data,onLoad,onView},ref){
 ...
 useEffect(()=>{if(!coordinator)return;const read=(event:Event)=>{(event as CustomEvent).detail.resolve(coordinator.getData().profile)};const save=(event:Event)=>{const {profile,before,resolve}=(event as CustomEvent).detail;try{const current=coordinator.getData(),id=coordinator.stage('offer',{...current,profile:before},{...current,profile},'Coaching profile');if(id)void coordinator.apply(id).then(resolve);else resolve(true)}catch{resolve(false)}};window.addEventListener('stride:profile-read',read);window.addEventListener('stride:profile-save',save);return()=>{window.removeEventListener('stride:profile-read',read);window.removeEventListener('stride:profile-save',save)}},[coordinator]);

 useImperativeHandle(ref,()=>({stage:(action,before,next,label)=>{if(!coordinator)throw new Error('Your account is still loading.');return coordinator.stage(action,before,next,label)},apply:id=>coordinator?.apply(id)||Promise.resolve(false),saveNow:async next=>{if(!coordinator)return false;const id=coordinator.stage('progress',data,next,'Progress updates');return id?coordinator.apply(id):true}}),[coordinator,data]);
