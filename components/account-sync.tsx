...
export const AccountSync=forwardRef<AccountSyncHandle,{data:Data;onLoad:(data:Data)=>void;onView?:(target:string)=>void}>(function AccountSync({data,onLoad,onView},ref){
 ...
 useEffect(()=>{if(!coordinator)return;const apply=async(event:Event)=>{const detail=(event as CustomEvent<{action:Operation['action'];before:Data;next:Data;label:string;resolve:(saved:boolean)=>void}>).detail;try{const id=coordinator.stage(detail.action,detail.before,detail.next,detail.label);if(!id){detail.resolve(true);return}const saved=await coordinator.apply(id);if(saved){coordinator.discard(id);toast.success('Saved to your account')}detail.resolve(saved)}catch{detail.resolve(false)}};window.addEventListener('stride:apply-coach-change',apply);return()=>window.removeEventListener('stride:apply-coach-change',apply)},[coordinator]);
 ...
 async function signOut(){
  if(!account?.signOutUrl)return;
  if(account.accountType==='guest'&&!window.confirm("If you sign out before creating an account, you may lose access to this progress. Sign out anyway?"))return;
  try{
   const response=await fetch(account.signOutUrl,{method:'POST'});
   if(!response.ok)throw new Error();
   window.location.assign('/');
  }catch{setTestError('Log out could not be completed. Please try again.')}
 }
 async function testAction(body:unknown){setTestBusy(true);setTestError('');try{if(!await coordinator?.flush())throw new Error('Finish saving your pending edits before changing the test workspace.');const response=await fetch('/api/test-workspace',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),result=await response.json() as {error?:string};if(!response.ok)throw new Error(result.error||'Testing could not be updated.');location.reload()}catch(e){setTestError(e instanceof Error?e.message:'Testing could not be updated.')}finally{setTestBusy(false)}}
 return <>
 {account?.testWorkspace&&<section className="test-workspace-banner" role="status"><div><FlaskConical size={20}/><span><strong>Test workspace</strong><small>Everything here is disposable. Your live tracking data is isolated.</small></span></div><div className="test-workspace-controls"><div className="test-clock"><strong>Test today: {fitnessNow().toLocaleDateString()}</strong><small>{dayOffset(document.cookie)} days ahead · does not create workouts</small><div>{([1,7,30] as const).map(days=><button key={days} className="secondary" disabled={testBusy} onClick={()=>void testAction({action:'clock',advance:days})}>+{days} {days===1?'day':'days'}</button>)}<button className="text-button" disabled={testBusy} onClick={()=>void testAction({action:'clock',advance:0})}>Use real date</button></div></div><label>Load scenario<select value={scenario} disabled={testBusy} onChange={e=>setScenario(e.target.value as TestScenario)}>{testScenarios.map(item=><option value={item.id} key={item.id}>{item.label}</option>)}</select></label><button className="secondary" disabled={testBusy} onClick={()=>void testAction({action:'scenario',scenario})}>{testBusy?'Loading…':'Load & reset test data'}</button><button className="primary" disabled={testBusy} onClick={()=>void testAction({action:'switch',mode:'live'})}>Return to live account</button></div>{testError&&<p role="alert">{testError}</p>}</section>}
 ...
 {state?.ready&&state.operations.length>0&&<><button className="primary coach-review-dock" onClick={()=>setReviewOpen(true)}>{pendingCount?`Review coach changes · ${pendingCount}`:'View saved coach changes'}</button><Dialog open={reviewOpen} onOpenChange={setReviewOpen}><DialogContent className="app-dialog coach-review-dialog"><DialogHeader><DialogTitle>Coach changes</DialogTitle><DialogDescription>{pendingCount?'Review what your coach proposed. Changes are saved only when you apply them.':'Your confirmed coach changes.'}</DialogDescription></DialogHeader><div id="coach-account-actions" className="coach-review-list">{state.operations.map(op=><CoachBoundary key={op.id}><CoachOperationCard op={op} data={data} onEditSet={(id,weight,reps)=>coordinator?.editSetProposal(id,weight,reps)} onApply={async id=>{if(!coordinator)return;if(await coordinator.apply(id)){coordinator.discard(id);toast.success('Saved to your account');if(!coordinator.getSnapshot().operations.some(op=>!op.receipt))setReviewOpen(false)}}} onDiscard={id=>void discard(id)} onView={target=>{setReviewOpen(false);onView?.(target)}}/></CoachBoundary>)}</div><button className="secondary full" onClick={()=>setReviewOpen(false)}>Back to what I was doing</button></DialogContent></Dialog></>}
 <button className={'account-button '+(account?.testWorkspace?'test-active':'')} aria-label="Account" onClick={()=>setOpen(true)}>{account?.testWorkspace?<FlaskConical size={18}/>:account?.name?.charAt(0)||<UserRound size={18}/>}</button><Dialog open={open} onOpenChange={setOpen}><DialogContent className="app-dialog account-dialog"><DialogHeader><DialogTitle>{account?.testWorkspace?'Test workspace':account?.accountType==='guest'?'Guest account':'Your Stride account'}</DialogTitle><DialogDescription>{account?.testWorkspace?'Isolated from '+account?.email:account?.accountType==='guest'?'Your progress is associated with this browser/device. Create an account to protect it and use Stride across devices.':account?.email||'Account connection'}</DialogDescription></DialogHeader><p role="status">{state?.status}</p>{account?.accountType==='guest'&&<a className="primary full" href="/">Create an account</a>}{account?.isAdmin&&<button className="secondary full" disabled={testBusy} onClick={()=>void testAction({action:'switch',mode:account?.testWorkspace?'live':'test'})}><FlaskConical size={17}/>{account?.testWorkspace?'Return to live account':'Switch to test workspace'}</button>}{account?.isAdmin&&!account?.testWorkspace&&<p className="form-help">The test workspace has separate workouts, reviews, goals, nutrition, measurements, and coach actions.</p>}{account?.isAdmin&&<a className="secondary full" href="/admin">Admin · Users & activity</a>}<button className="secondary" onClick={backup}>Download recovery copy</button>{testError&&<p className="plan-error" role="alert">{testError}</p>}{account?.signOutUrl&&<button className="secondary full" onClick={()=>void signOut()}>Log out</button>}</DialogContent></Dialog>
 </>;
});
