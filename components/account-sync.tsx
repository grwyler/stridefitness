...
export type AccountSyncHandle={stage:(action:Operation['action'],before:Data,next:Data,label:string,operationId?:string)=>string|null;apply:(id:string)=>Promise<boolean>;flush:()=>Promise<boolean>;saveNow:(next:Data)=>Promise<boolean>};
export const AccountSync=forwardRef<AccountSyncHandle,{data:Data;onLoad:(data:Data)=>void;onView?:(target:string)=>void}>(function AccountSync({data,onLoad,onView},ref){
 ...
 useImperativeHandle(ref,()=>({stage:(action,before,next,label,operationId)=>{if(!coordinator)throw new Error('Your account is still loading.');return coordinator.stage(action,before,next,label,operationId)},apply:id=>coordinator?.apply(id)||Promise.resolve(false),flush:()=>coordinator?.flush()||Promise.resolve(false),saveNow:async next=>{if(!coordinator)return false;const id=coordinator.stage('progress',data,next,'Progress updates');return id?coordinator.apply(id):true}}),[coordinator,data]);
 ...
 {state?.ready&&<p role="status" className="muted">{state.status}</p>}
