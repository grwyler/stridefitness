...
export function FirstRunCoach({onPlan,onWorkout,onExplore}:{onPlan:()=>void;onWorkout:()=>void;onExplore:()=>void}){
 ...
 return <>
  <section className="panel first-run-coach" aria-labelledby="first-run-title">
   <div className="first-run-copy"><span className="first-run-icon"><Sparkles size={22}/></span><div><span className="eyebrow">WELCOME TO STRIDE</span><h2 id="first-run-title">{ai==="enabled"?"Let’s make your first workout useful.":"Start wherever feels right."}</h2><p>{ai==="enabled"?"Describe the routine you already use, or ask for a starting point. Review a real workout before you save it.":"Log your own training, create a reusable workout, or connect AI whenever you want coaching."}</p></div></div>
   <div className="first-run-actions">{ai==="enabled"&&<button className="primary" onClick={onPlan}>Build or describe my workout <ArrowRight size={17}/></button>}<button className={ai==="enabled"?"secondary":"primary"} onClick={onWorkout}>Log my own workout</button><button className="text-button" onClick={()=>{setStep(0);setOpen(true)}}>Show me around</button></div>
