'use client';
import {adminActionResponseSchema,responseError} from '@/lib/api-responses';
...
export function ResetUserButton({userId,name,aiRevoked}:{userId:string;name:string;aiRevoked:boolean}){
 ...
 async function change(action:Action){
  ...
  try{const response=await fetch('/api/admin/reset-user',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId,action})}),body=await response.json();if(!response.ok)throw new Error(responseError(body,'Account change failed'));const acknowledgement=adminActionResponseSchema.safeParse(body);if(!acknowledgement.success||acknowledgement.data.action!==action)throw new Error('The account change could not be confirmed. Reload the user list to check its current state before trying again.');window.location.reload()}
