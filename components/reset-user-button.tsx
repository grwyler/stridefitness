...
export function ResetUserButton({userId,name,aiRevoked}:{userId:string;name:string;aiRevoked:boolean}){
 ...
 return <div className={styles.reset}>
  <div className={styles.aiStatus}><span>AI access: <strong>{aiRevoked?'Revoked':'Granted'}</strong></span><button className={aiRevoked?styles.grant:styles.revoke} disabled={!!busy} onClick={()=>void change(aiRevoked?'grant_ai':'revoke_ai')}>{busy==='grant_ai'||busy==='revoke_ai'?spinner:aiRevoked?<ShieldCheck size={16}/>:<ShieldOff size={16}/>} {aiRevoked?'Grant AI access':'Revoke AI access'}</button></div>
