...
export function AIConnection(){
 ...
 const [auth,setAuth]=useState<'checking'|'signed-in'|'sign-in'|'unavailable'|'revoked'>('checking');
 const [shared,setShared]=useState(false),[canShare,setCanShare]=useState(false);
 ...
 async function checkConnection(){
  ...
  try{
   ...
   if(body.accessRevoked){setConnected(false);setShared(false);setCanShare(false);setAuth('revoked');return}
   setConnected(body.connected);setShared(!!body.shared);setCanShare(!!body.canShare);setAuth(body.canShare||!body.shared?'signed-in':'unavailable');
 ...
 async function updateSharing(){setBusy(true);setError('');setNotice('');try{const response=await fetch('/api/shared-ai',{method:shared?'DELETE':'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(15000)}),result=await response.json();if(!response.ok)throw new Error(result.error||'Unable to update shared AI.');setShared(result.shared);setConnected(true);setNotice(result.shared?'Shared AI is on. Visitors can now use your server-side connection.':'Shared AI is off. Visitors will need their own connection.')}catch(e){setError(e instanceof Error?e.message:'Unable to update shared AI.')}finally{setBusy(false)}}
 ...
 {auth==='revoked'&&<div className="connection-notice">AI access has been disabled for this Stride account. Contact the site owner if you think this is a mistake.</div>}
 {auth==='unavailable'&&shared?<div className="connection-notice">AI is included for visitors. No personal connection is needed.</div>:auth==='unavailable'&&<button className="secondary" onClick={()=>void checkConnection()}>Try again</button>}
 ...
 {canShare&&connected&&<div className="shared-ai-control"><div><strong>AI for all visitors</strong><p>Uses your saved key. Limited to 20 AI requests per visitor each day.</p></div><button className={shared?'secondary':'primary'} disabled={busy} onClick={()=>void updateSharing()}>{shared?'Turn off':'Share my AI connection'}</button></div>}{error&&<p className="plan-error" role="alert">{error}</p>}{notice&&<p className="connection-notice" role="status">{notice}</p>}</DialogContent></Dialog></>
}
