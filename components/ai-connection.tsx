...
export function AIConnection(){
 ...
 const [shared,setShared]=useState(false),[canShare,setCanShare]=useState(false),[personal,setPersonal]=useState(false),[included,setIncluded]=useState(false);
 ...
 async function checkConnection(){
  ...
  try{
   ...
   const body=await response.json() as {signInUrl?:string;error?:string;connected:boolean;shared:boolean;canShare:boolean;personal:boolean;included:boolean};
   ...
   setConnected(body.connected);setShared(!!body.shared);setCanShare(!!body.canShare);setPersonal(!!body.personal);setIncluded(!!body.included);setAuth('signed-in');
 ...
 async function update(remove=false){
  ...
  try{
   ...
   const result=await response.json() as {signInUrl?:string;error?:string;connected:boolean};
   if(response.status===401){requireSignIn(result);return}
   if(!response.ok)throw new Error(result.error||'Unable to save connection.');
   setKey('');await checkConnection();setNotice(remove?'Personal key removed.':'Key saved. Your coach will use this connection.');
 ...
 async function updateSharing(){setBusy(true);setError('');setNotice('');try{const response=await fetch('/api/shared-ai',{method:shared?'DELETE':'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(15000)}),result=await response.json() as {error?:string;shared:boolean};if(!response.ok)throw new Error(result.error||'Unable to update shared AI.');await checkConnection();setNotice(result.shared?'Your connection now funds accounts with complimentary AI.':'Shared AI is off. Users can still use their own keys.')}catch(e){setError(e instanceof Error?e.message:'Unable to update shared AI.')}finally{setBusy(false)}}
 return <><button className="secondary" onClick={()=>{setOpen(true);setError('');setNotice('');void checkConnection()}}><KeyRound size={16}/>{connected?'Manage AI':'Connect AI'}</button><Dialog open={open} onOpenChange={value=>{if(busy)return;setOpen(value);setKey('');setError('');setNotice('')}}><DialogContent className="app-dialog ai-connect"><DialogHeader><DialogTitle>{connected?'Your AI connection':'Connect AI'}</DialogTitle><DialogDescription>Choose how to power your coach.</DialogDescription></DialogHeader>
 ...
 {auth==='signed-in'&&<><p className="connection-notice">{personal?'Your coach is using your personal API key.':included&&shared?'AI is included for your account. No setup needed.':included?'Your complimentary AI connection is temporarily unavailable. You can connect your own key below.':'Complimentary AI is not included for your account. Connect your own key to keep using the coach.'}</p><details open={!connected||canShare}><summary>{personal?'Manage personal key':connected?'Use your own API key':'Connect your own API key'}</summary><ol className="connection-steps"><li>Open <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener noreferrer">OpenAI API keys</a> if you need a key.</li><li>Select “Create new secret key,” name it “Stride,” and copy the key. If you already copied a key, use that one.</li><li>Paste it below and select Save key.</li></ol><form onSubmit={e=>{e.preventDefault();void update()}}><label htmlFor="ai-secret">{personal?'Replace API key':'API key'}<input id="ai-secret" type="password" autoComplete="off" spellCheck={false} autoCapitalize="none" value={key} disabled={busy} onChange={e=>setKey(e.target.value)} placeholder="sk-…" maxLength={2048} required/></label><p className="form-help">Stored encrypted on the server for your signed-in account. The saved key is never shown again or included in your planning chat.</p><p className="form-help">OpenAI API usage is billed separately from ChatGPT. You may need to add credits in <a href="https://platform.openai.com/settings/organization/billing" target="_blank" rel="noopener noreferrer">API billing</a>.</p><button className="primary full" disabled={busy||!key.trim()}>{busy?<Loader2 size={16} className="plan-spin"/>:<KeyRound size={16}/>}Save key</button></form>{personal&&<button className="text-button full" disabled={busy} onClick={()=>void update(true)}>Remove personal key</button>}</details></>}
 {canShare&&personal&&<div className="shared-ai-control"><div><strong>Fund complimentary AI</strong><p>Uses your saved key for eligible accounts. Existing daily limits still apply.</p></div><button className={shared?'secondary':'primary'} disabled={busy} onClick={()=>void updateSharing()}>{shared?'Turn off':'Share my AI connection'}</button></div>}{error&&<p className="plan-error" role="alert">{error}</p>}{notice&&<p className="connection-notice" role="status">{notice}</p>}</DialogContent></Dialog></>
}
