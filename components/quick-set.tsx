...
export function QuickSet({set:s,index,exercise,next,onPatch,onDelete,onAdvance}:{set:SetLog;index:number;exercise:string;next:boolean;onPatch:(patch:Partial<SetLog>)=>void;onDelete:()=>void;onAdvance:()=>void}){
 const label=`${exercise} set ${index+1}`;
 function record(status:SetLog['status']){onPatch({status});if(next&&status!=='pending')onAdvance()}
