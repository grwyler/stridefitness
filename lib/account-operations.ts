...
const goal=z.object({id,title:z.string().min(1).max(100),kind:z.enum(['measurement','sessions','strength','compound','milestone']),unit:z.string().min(1).max(20),start:z.number().finite(),target:z.number().finite(),started:z.string(),deadline:z.string(),archived:z.boolean(),exerciseId:id.optional(),checks:z.array(z.object({id,date:z.string(),value:z.number().finite(),note:z.string()}))}).passthrough();
...
const allowed:Record<Operation['action'],string[]>={plan:['templates','coachPlanner'],template:['templates'],session:['workouts'],set:['workouts'],exercise:['exercises'],progress:['goals','nutrition','activityEnergy'],offer:['goals','nutrition','profile','bodyMeasurements','strengthProfile','coachOffers','overrides']};
