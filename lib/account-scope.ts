...
export function isTestOperator(user:ChatGPTUser){return user.accountType==='test'&&user.userId==='test:automation'}
...
export async function canUseTestWorkspace(user:ChatGPTUser){return isTestOperator(user)||await isOwner(user)}
...
export async function accountScope(user:ChatGPTUser,request?:Request){
 const primary=await accountDataId(user),test=isTestOperator(user)||(!!request&&requestedTestWorkspace(request)&&await isOwner(user));
