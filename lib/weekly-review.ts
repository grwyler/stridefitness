...
export function analyzeWeek(d:Data,day:string,priorReviews:PriorReview[]=[]){
 ...
 for(const goal of goals.filter(goal=>goal.kind==='strength'&&goal.exerciseId)){
  ...
  candidates.push({key:'goal:'+goal.id,kind:'continue',priority:110,text:`Goal achieved: set your next ${exercise.name} goal at ${nextTarget} ${goal.unit}.`,why:`Your recorded ${exercise.name} estimate is ${progress.current} ${goal.unit}, above the ${goal.target} ${goal.unit} goal. Choose a higher goal before treating routine performance as a reason to hold.`,evidenceKeys:[evidenceKey]});
 }
