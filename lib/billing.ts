...
export async function createTopUpCheckout(user:ChatGPTUser,origin:string){
 const id=await accountDataId(user);let row=await db().prepare('SELECT stripe_customer_id FROM billing_accounts WHERE user_id = ?').bind(id).first<{stripe_customer_id:string|null}>();let customer=row?.stripe_customer_id||null;
