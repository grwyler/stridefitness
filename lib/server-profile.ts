...
export async function getCoachContext(user:ChatGPTUser){
 const row=await adminDatabase().prepare('SELECT data FROM user_training_data WHERE user_id = ?').bind(await accountDataId(user)).first<{data:string}>();if(!row)return null;
