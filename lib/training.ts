...
export type DailyReadiness={date:string;sleep:'low'|'okay'|'great';stress:'low'|'okay'|'high';soreness:'low'|'okay'|'high';energy:'low'|'okay'|'high'};
export type Data={coachChats?:Record<string,import('../components/coach-memory').CoachMessage[]>;coachOffers?:Record<string,import('./coaching-updates').CoachingUpdates>;coachPlanner?:{messages:import('./plan').PlanMessage[];plan:import('./plan').GeneratedPlan|null;ids:string[];draft?:boolean};profile?:import('./profile').CoachingProfile;user:{id:string;name:string};exercises:Exercise[];workouts:Workout[];templates:Template[];overrides:Record<string,Target>;dark:boolean;catalogVersion?:number;goals?:Goal[];strengthProfile?:StrengthProfile;nutrition?:Nutrition;bodyMeasurements?:BodyCheckIn[];activityEnergy?:import('./activity-energy').ActivityEnergy;recoveryOverrides?:{group:string;reportedAt:string}[];dailyReadiness?:DailyReadiness[]};
