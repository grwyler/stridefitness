...
export const planSchema=z.object({saveUpdates:coachingUpdatesSchema.optional().default(null),
 ...
 progress:z.object({
  goal:z.object({title:z.string().min(1).max(100),kind:z.enum(['measurement','daily','sessions','strength','compound','milestone']),unit:z.string().min(1).max(20),start:z.number(),target:z.number(),deadline:z.string().max(20).nullable(),exerciseId:z.string().max(100).nullable(),dailyMetric:z.enum(['hydration','activeCalories','protein','calorieIntake']).nullable().optional().default(null)}).nullable(),
...
export function applyProgressProposal(data:Data,proposal:ProgressProposal):Data{
 ...
 if(proposal.goal){
  ...
  const goal:Goal={id:uid(),title:g.title,kind:g.kind,unit:g.unit,start,target:g.target,started:today,deadline:g.deadline||'',archived:false,checks:[],...(g.exerciseId?{exerciseId:g.exerciseId}:{}),...(g.dailyMetric?{dailyMetric:g.dailyMetric}:{})};
...
export function coachingContext(data:Data){
 ...
 return {nutrition:data.nutrition?{date:localDay(),dayComplete:isDayComplete(data.dayCompletions,localDay()),calorieTarget:data.nutrition.calorieTarget,proteinTarget:data.nutrition.proteinTarget,activityCalorieAdjustment:data.nutrition.activityCalorieAdjustment??0,...nutritionTotals(data.nutrition.entries,localDay())}:undefined,bodyMeasurements:(data.bodyMeasurements||[]).slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,12).map(({date,weight,bodyFat})=>({date,weight,bodyFat})),activityEnergy:{templates:(data.activityEnergy?.templates||[]).map(({id,...x})=>x),recentLogs:(data.activityEnergy?.logs||[]).slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,12).map(({name,date,durationMinutes,intensity,caloriesBurned})=>({name,date,durationMinutes,intensity,caloriesBurned})),completedDays:(data.dayCompletions||[]).slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,14).map(row=>row.date)},completedSessions:completed.length,recent,goals:(data.goals||[]).filter(g=>!g.archived).slice(0,10).map(g=>({title:g.title,kind:g.kind,exerciseId:g.exerciseId,dailyMetric:g.dailyMetric,unit:g.unit,start:g.start,target:g.target,current:goalProgress(g,data).current,deadline:g.deadline}))};
}
