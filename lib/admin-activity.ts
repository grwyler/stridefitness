...
export async function accountDataId(user:ChatGPTUser){if(user.accountId)return user.accountId;if(!user.email)throw new Error('Account identity is unavailable');return Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(user.email.trim().toLowerCase()))),b=>b.toString(16).padStart(2,'0')).join('')}
