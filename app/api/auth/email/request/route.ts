...
export async function POST(request:Request){
 ...
 try{await ensureIdentityTables();const now=new Date(),recent=await config.DB.prepare("SELECT COUNT(*) AS count FROM email_login_challenges WHERE email=? AND requested_at>?").bind(email,new Date(now.getTime()-15*60_000).toISOString()).first<{count:number}>();if((recent?.count||0)>=3)return Response.json({ok:true});const token=crypto.randomUUID()+crypto.randomUUID(),tokenHash=await hash(token),expiry=new Date(now.getTime()+15*60_000).toISOString(),origin=new URL(request.url).origin,link=`${origin}/api/auth/email/verify?token=${encodeURIComponent(token)}`;
