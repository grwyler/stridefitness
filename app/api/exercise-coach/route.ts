...
export async function POST(request:Request){
 ...
 try{
  ...
  const text=result.output?.flatMap(o=>o.content||[]).filter(c=>c.type==='output_text').map(c=>c.text||'').join('');if(paid)await chargeAIUsage(user,settings.OPENAI_MODEL||'gpt-4.1-mini',result.usage);const reply=JSON.parse(text||'');reply.saveUpdates=coachingUpdatesSchema.parse(reply.saveUpdates??null);return json(reply);
