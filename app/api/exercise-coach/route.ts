...
export async function POST(request:Request){
 ...
 try{
  ...
  const text=result.output?.flatMap(o=>o.content||[]).filter(c=>c.type==='output_text').map(c=>c.text||'').join('');if(paid)await chargeAIUsage(user,settings.OPENAI_MODEL||'gpt-4.1-mini',result.usage);return json(JSON.parse(text||''));
