...
import {hasUserConnection,saveUserConnection,removeUserConnection} from '@/lib/ai-connection';
...
export async function GET(request:Request){
 ...
 try{return json({connected:await hasUserConnection(user)})}catch{return json({error:'Connection settings are temporarily unavailable. Please try again.'},503)}
}
