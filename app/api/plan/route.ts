...
export async function POST(request:Request){
 if(!await getChatGPTUser())return json({error:'Please sign in to Stride to create a plan.'},401);
 ...
 if(!settings.OPENAI_API_KEY)return json({error:'AI planning is not connected yet. Ask the site owner to finish connecting OpenAI.'},503);
 try{
