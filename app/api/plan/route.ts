...
const nutrition=object({calorieTarget:{type:['integer','null']},proteinTarget:{type:['integer','null']}});
const outputSchema=object({message:{type:'string'},workouts:{type:'array',items:object({name:{type:'string'},description:{type:'string'},entries:{type:'array',items:object({exerciseId:{type:'string'},sets:{type:'integer'},reps:{type:'integer'},weight:{type:['number','null']}})}})},progress:{type:['object','null'],properties:{goal:{...goal,type:['object','null']},nutrition:{...nutrition,type:['object','null']}},required:['goal','nutrition'],additionalProperties:false}});
...
export async function POST(request:Request){
 ...
 try{
  ...
  const progressInstructions='The response must also include progress. Set progress to null unless the user asks for a tracked goal or nutrition targets, or the proposed plan clearly benefits from offering them. When useful, progress may propose one goal and/or daily calorie and protein targets. Ask one focused question first and return no workouts and progress null when essential target details are missing. Never invent body weight, calorie needs, protein needs, timeline, or measurement. Strength goals use a catalog short ID in exerciseId. Compound goals use no exercise ID and track the combined important compound lifts. Use sessions or milestone for adherence goals. Use measurement only for user-supplied measurements. Use conservative targets and do not prescribe extreme restriction. Never say targets were saved; the application presents them for explicit approval.';
  const compactTraining=training?{...training,recent:training.recent.map(e=>({...e,exerciseId:idToAlias.get(e.exerciseId)||e.exerciseId})),coachInstructions:progressInstructions}:{coachInstructions:progressInstructions};
  if(user)await recordActivity(user,true);const coachingProfile=await getCoachingProfile(user);if(!coachingProfile)return Response.json({error:'Complete your coaching interview before continuing.'},{status:403});
