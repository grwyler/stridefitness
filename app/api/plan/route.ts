...
const inputSchema=z.object({messages:z.array(z.object({role:z.enum(['user','assistant']),content:z.string().min(1).max(4000),photo:z.string().max(1500000).regex(/^data:image\/jpeg;base64,\/9j\/[A-Za-z0-9+/]*={0,2}$/).optional()})).min(1).max(12),exercises:z.array(z.object({id:z.string().max(100),name:z.string().max(100),category:z.string().max(100)})).min(1).max(1000),currentPlan:planSchema.nullable(),training:z.object({goals:z.array(z.object({title:z.string().max(100),kind:z.enum(['measurement','sessions','milestone']),unit:z.string().max(20),start:z.number(),target:z.number(),current:z.number(),deadline:z.string().max(20)})).max(10).optional(),completedSessions:z.number().int().min(0),recent:z.array(z.object({exerciseId:z.string().max(100),date:z.string().max(50),weight:z.number().min(0).nullable(),reps:z.number().min(0).nullable(),completedSets:z.number().int().min(0),failedSets:z.number().int().min(0),difficulty:z.string().max(50)})).max(8)}).optional()});
