...
import {analyzeWeek,shiftDay,targetSchema,validDay,type WeeklyReview} from '@/lib/weekly-review';
import {changes,validateOperation,type Operation} from '@/lib/account-operations';
import {loadReview,recentReviews,reviewDatabase,reviewReceipt,selectRecommendation} from '@/lib/weekly-review-server';
...
export async function GET(request:Request){const user=await getChatGPTUser(request);if(!user)return json({error:'Sign in to open your weekly review.'},401);try{const accountId=await accountDataId(user),recent=await recentReviews(accountId);return json({accountId,review:recent[0]?{...recent[0].review,receipt:recent[0].receipt}:null})}catch{return json({error:'Your review could not be loaded. Please retry.'},503)}}
export async function POST(request:Request){
 ...
 try{
  ...
  if(value.intent==='generate'){
   ...
   const existing=await loadReview(userId,value.id);if(existing)return json({accountId:userId,review:{...existing,receipt:await reviewReceipt(userId,existing)}});
   const recent=await recentReviews(userId);
   if(recent[0]?.review.day===value.day&&recent[0].review.revision===revision)return json({accountId:userId,review:{...recent[0].review,receipt:recent[0].receipt}});
   const analysis=analyzeWeek(data,value.day,recent),selected=await selectRecommendation(analysis,user,request);
   const review:WeeklyReview={id:value.id,createdAt:new Date().toISOString(),day:value.day,start:analysis.start,revision,summary:analysis.summary,standout:analysis.standout,coverage:analysis.coverage,observations:analysis.observations,keepDoing:analysis.keepDoing,evidence:analysis.evidence,prior:analysis.prior,...selected,feedback:'open'};
  ...
  if(value.intent==='feedback'){
   const result=await db.prepare("UPDATE weekly_reviews SET content=json_set(content,'$.feedback',?) WHERE id=? AND user_id=?").bind(value.feedback,value.id,userId).run();if(!result.meta.changes)return json({error:'Feedback could not be kept. Please retry.'},409);
   return json({accountId:userId,review:{...review,feedback:value.feedback,receipt:await reviewReceipt(userId,review)}});
  }
  ...
  if(revision!==review.revision)return json({error:'Your training changed since this review. Keep this review and refresh it before proposing a target.'},409);
  ...
  await db.prepare("UPDATE weekly_reviews SET content=json_set(content,'$.prepared',json(?)) WHERE id=? AND user_id=? AND json_extract(content,'$.prepared') IS NULL").bind(JSON.stringify(prepared),review.id,userId).run();
