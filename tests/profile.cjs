...
vm.runInNewContext(ts.transpileModule(fs.readFileSync('app/api/profile/route.ts','utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS}}).outputText,{exports:routeModule.exports,module:routeModule,Response,Request,URL,Date,require:name=>name==='@/app/chatgpt-auth'?{getChatGPTUser:async()=>({email:'sam@test'})}:name==='@/lib/server-profile'?{getCoachingProfile:async()=>null}:name==='@/lib/profile'?{profileCompleteSchema}:name==='@/lib/admin-activity'?{adminDatabase:()=>adapter,accountDataId:async()=>'sam'}:name==='@/lib/account-scope'?{accountScope:async()=>({id:'sam',mode:'live'})}:name==='@/lib/training'?{initialData:()=>({workouts:[],goals:[]})}:require(name)});
