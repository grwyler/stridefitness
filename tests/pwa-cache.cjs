...
(async()=>{
 let install;handlers.install({waitUntil:p=>install=p});await install;assert.equal(writes.length,5);
 ...
 let response;handlers.fetch({request:{url:'https://stridefitness.org/admin',method:'GET',mode:'navigate'},respondWith:p=>response=p});await response;assert.equal(writes.length,5,'HTML must not be cached');
 fail=true;handlers.fetch({request:{url:'https://stridefitness.org/admin',method:'GET',mode:'navigate'},respondWith:p=>response=p});assert.equal(await response,cache.get('/offline.html'));fail=false;
 handlers.fetch({request:{url:'https://stridefitness.org/_next/static/chunks/framework-D_rUT4EX.js',method:'GET',mode:'cors'},respondWith:p=>response=p});await response;assert.equal(writes.length,6,'hashed public JS cached');
